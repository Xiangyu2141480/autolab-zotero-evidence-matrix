...
Release verification repair evidence: `build==1.5.0` (the currently installable stable release) is a pinned development dependency and CI invokes `python -m build`; generated `build/` and `dist/` directories are ignored. The output-directory `OSError` subprocess regression test immediately reported `1 passed, 15 deselected` because the existing CLI boundary already produced `Unable to write output file`; no behavioral implementation was needed. After installing updated dev dependencies in `.venv`, distribution build generated both the sdist and wheel successfully, the full suite reported `16 passed in 0.74s`, Ruff reported `All checks passed!`, and pip-audit reported `No known vulnerabilities found` (skipping only the unpublished local package). The package license metadata now uses the current SPDX string form `MIT`, avoiding build-backend deprecation warnings.
